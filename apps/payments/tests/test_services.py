...
class PaymentServiceTests(TestCase):
    ...
    def test_get_gateway_returns_correct_gateway(self):
        gateway = PaymentService.get_gateway("chapa")
        self.assertIsNotNone(gateway)

    def test_get_gateway_invalid_provider(self):
        with self.assertRaises(ValueError):
            PaymentService.get_gateway("bitcoin")

    @patch("apps.payments.services.Payment.objects.create")
    ...
    @patch("apps.payments.services.PaymentService.get_gateway")
    def test_initialize_payment(self, mock_gateway):
        gateway = Mock()
        ...
        result = PaymentService.initialize_payment(self.payment)
    ...
    def test_verify_payment_success(self, mock_gateway):
        ...
        result = PaymentService.verify_payment(self.payment)
    ...
    @patch("apps.payments.services.PaymentService.get_gateway")
    def test_verify_payment_wrong_amount(self, mock_gateway):
        gateway = Mock()
        gateway.verify_payment.return_value = {
            "verified": True,
            "amount": "100.00",
        ...
        with self.assertRaises(AmountMismatchError):
            PaymentService.verify_payment(self.payment)

    @patch("apps.payments.services.PaymentService.get_gateway")
    def test_verify_payment_wrong_currency(self, mock_gateway):
        gateway = Mock()
        gateway.verify_payment.return_value = {
            "verified": True,
            "amount": "250.00",
    ...
    @patch("apps.payments.services.PaymentService.get_gateway")
    def test_gateway_verification_failed(self, mock_gateway):
        gateway = Mock()
        gateway.verify_payment.return_value = {
            "verified": False,
        }
    ...
    def test_cannot_complete_twice(self):
        ...
        with self.assertRaises(InvalidPaymentStateError):
            PaymentService.mark_completed(self.payment)

    @patch("apps.payments.services.PaymentService.get_gateway")
    def test_refund_payment(self, mock_gateway):
        gateway = Mock()
        gateway.refund_payment.return_value = {"success": True}
        mock_gateway.return_value = gateway

        result = PaymentService.refund_payment(self.payment)
